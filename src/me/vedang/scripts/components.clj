...
(def cli-options
  [["-e" "--exclude-paths EXCLUDE-DIR"
   ...
   [nil "--stdin"
    "Accept list of files from standard input. Ignore C1/C2 args in this case. List should be in the same format as generated by 'git diff --name-only'"]
   ["-h" "--help"
...
(defn clj-files
  ...
  (let [exclude-pred (apply some-fn
                            (map (fn [subs] (fn [s] (cs/starts-with? s subs)))
                                 exclude-paths))
        include-pred (apply some-fn
                            (map (fn [subs] (fn [s] (cs/ends-with? s subs)))
                                 [".clj" ".cljc" ".cljs"]))]
    (transduce (comp (remove exclude-pred) (filter include-pred))
               conj
               []
               files)))
...
(defn input
  ...
  [earliest latest stdin exclude-paths]
  (cond
    ;; Ignore everything else, just read files from input
    stdin (-> *in* slurp (cs/split #"\n") (clj-files exclude-paths))
    ;; Else use git to find the changed files
    (zero? (:exit (shell/sh "which" "git")))
    (-> (shell/sh "git" "diff" earliest latest "--name-only")
        :out
        (cs/split #"\n")
        (clj-files exclude-paths))
    ;; Else can't find files.
    :else []))
...
(defn print-components
  [{:keys [earliest latest timeout exclude-paths source-paths verbose stdin]}]
  ...
  (let [files (input earliest latest stdin exclude-paths)]
    (when (seq files)
